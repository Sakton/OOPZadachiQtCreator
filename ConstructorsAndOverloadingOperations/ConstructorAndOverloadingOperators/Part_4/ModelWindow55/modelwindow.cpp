...
ModelWindow::ModelWindow( const Monitor& monitor, const std::string& nm,
                          const ModelWindow::Pair& cup, int32_t w, int32_t h,
                          ModelWindow::Color cl, bool vsbl, bool fr,
                          const Cursor40B& crr )
    : monitor_ { monitor }, name_ { nm }, cursor_( crr ) {
...
Cursor40B ModelWindow::cursor() const
{
    return cursor_;
}

void ModelWindow::setCursor(const Cursor40B &cursor)
{
    cursor_ = cursor;
}
