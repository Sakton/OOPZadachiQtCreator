...
void Account32::Money24::Display( ) {
  std::cout << static_cast< std::string >( *this ) << std::endl;
}

Account32::Money24::operator double( ) const {
  return double( rub ) + double( cop ) / 100.0;
}

void Read( Account32::Money24& b ) {
  int32_t aa = 0;
  int16_t bb = 0;
  std::cout << "Enter rub cop" << std::endl;
  std::cin >> aa >> bb;
  b = Account32::Money24( aa, bb );
}

Account32::Money24::operator std::string( ) const {
  std::stringstream ss;
  ss << std::fixed << rub << ",";
...
Account32::Money24::Money24( int32_t rb, int16_t cp ) {
  if ( !( 0 <= cp && cp < 100 ) ) {
...
Account32::Money24::Money24( double sum )
    : Money24( sum, ( sum - static_cast< int16_t >( sum ) ) * 100 ) {}

Account32::Money24& Account32::Money24::operator+=( const Money24& b ) {
...
Account32::Money24& Account32::Money24::operator*=( double x ) {
  int64_t rurA = ( rub < 0 ) ? ( rub * 100 - cop ) : ( rub * 100 + cop );
  int64_t mul = rurA * x;
  rub = mul / 100;
  cop = std::abs( mul % 100 );
...
double Account32::Money24::operator/=( const Money24& b ) {
  int64_t rurA = ( rub < 0 ) ? ( rub * 100 - cop ) : ( rub * 100 + cop );
  int64_t rurB =
      ( b.rub < 0 ) ? ( b.rub * 100 - b.cop ) : ( b.rub * 100 + b.cop );
  if ( rurB == 0 ) {
...
Account32::Money24 operator+( const Account32::Money24& a,
                              const Account32::Money24& b ) {
  Account32::Money24 loc = a;
  loc += b;
  return loc;
}
...
Account32::Money24 operator*( const Account32::Money24& a, double x ) {
  Account32::Money24 loc = a;
  loc *= x;
  return loc;
}

double operator/( const Account32::Money24& a, const Account32::Money24& b ) {
  Account32::Money24 loc = a;
  return loc /= b;
}
...
bool operator==( const Account32::Money24& a, const Account32::Money24& b ) {
...
int32_t Account32::Money24::GetRub( ) const { return rub; }

uint8_t Account32::Money24::GetCop( ) const { return cop; }
