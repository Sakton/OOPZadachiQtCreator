...
//если передано большое число лучше прервать программу и выкинуть исключение, чем инициализировать другое значени еи его испльзовать в расчетах
//ошибку потом можно и не найти
Decimal::Decimal( int sz ) : decm{ nullptr }, count{ 0 }, negative{ false } {
	sz = ( 0 < sz && sz < MAX_SIZE ) ? sz : throw new std::runtime_error( "bad size num (ctor)" );
	decm = new Type[ sz + 1 ];
	for( int i = 0; i < sz; ++i ) decm[ i ] = '0';
	decm[ sz ] = '\0';
	count = sz;
}
...
Decimal::Decimal( const char * str ) : Decimal() {
	...
	if( lenth > MAX_SIZE ) throw new std::runtime_error( "bad size num (ctor)" ); //если строка большая то нет смысла брать часть, тк. то число само по себе больше и нужен другой тип
	if( !checkStringForCorrectness( str ) ) throw new std::runtime_error( "bad str ctor (ctor)" ); //если строка не корректна то выход
	Type * t = new Type[ lenth + 1 ];
	if( !t ) throw new std::runtime_error( "bad alloc memory (ctor)" ); //если память не выделилась выход
	int end = ( negative = ( str[ 0 ] == '-' ) ) ? 1 : 0; //минус уменьшает проход на 1 и присвоим значение негатив в 1 операторе
	for( int i = 0, j = lenth - 1; j >= end; --j, ++i ) { //пишем переворачивая строку
		t[ i ] = str[ j ];
	}
...
Decimal::Decimal( const std::string & str ) : Decimal( str.c_str() ) { } //просто делегирую в другой конструктор
...
bool operator != ( const Decimal & first, const Decimal & second ) {
	if( ( first.count != second.count ) || first.negative != second.negative ) return true;
	for( int i = first.count - 1; i >= 0; --i ) {
		if( first.decm[ i ] != second.decm[ i ] ) return true;
	}
	return false;
	//return !(std::strcmp(reinterpret_cast<const char*>(first.decm), reinterpret_cast<const char*>(second.decm)) == 0);
}
...
bool LessPoModule( const Decimal & first, const Decimal & second ) {
	...
	if( first.count == second.count ) {
		for( int i = first.count - 1; i >= 0; --i ) {
			...
			else if( first.decm[ i ] < second.decm[ i ] ) return true;
			else if( first.decm[ i ] > second.decm[ i ] ) return false;
		}
	}
	return false;
}
...
Decimal & Decimal::operator += ( const Decimal & other ) {
	...
	if( negative == false && other.negative == false ) {
		*this = ( *this <= other ) ? summa( other, *this ) : summa( *this, other );
	} else if( negative == true && other.negative == true ) {
		*this = ( *this < other ) ? summa( *this, other ) : summa( other, *this );
		negative = true;
	} else if( negative != other.negative ) {
		if( negative == true && other.negative == false && LessPoModule( *this, other ) ) {
			//если само отрицательно и больше по модулю
			*this = raznost( *this, other );
		} else if( negative == true && other.negative == false && !LessPoModule( *this, other ) ) {
			//если само отрицательно и меньше по модулю
			*this = raznost( other, *this );
			negative = other.negative;
		} else if( negative == false && other.negative == true && LessPoModule( *this, other ) ) {
			//если само положительное, а второе отрицательно но первое меньше по модулю
			*this = raznost( other, *this );
			negative = other.negative;
		} else if( negative == false && other.negative == true && !LessPoModule( *this, other ) ) {
			//если само положительное, а второе отрицательно но первое больше по модулю
			*this = raznost( *this, other );
		}
	}
	Trim();
...
Decimal & Decimal::operator -= ( const Decimal & other ) {
	...
	//2. Если оба отрицательные
	//3. Разных знаков
	if( negative == false && other.negative == false && LessPoModule( *this, other ) ) {
		//оба положительные но само меньше по модулю
		*this = raznost( other, *this );
		negative = true;
	} else if( negative == false && other.negative == false && !LessPoModule( *this, other ) ) {
		//оба положительные но само больше по модулю
		*this = raznost( *this, other );
	} else if( negative == true && other.negative == true && LessPoModule( *this, other ) ) {
		...
		*this = raznost( *this, other );
		...
		//если само положительно другой отрицателен и само по модулю меньше другого
		*this = summa( other, *this );
	} else if( negative == true && other.negative == false && LessPoModule( *this, other ) ) {
		//если само отрицательно другой положителен и само по модулю меньше другого
		*this = summa( other, *this );
	} else if( negative == false && other.negative == true && !LessPoModule( *this, other ) ) {
		//если само положительно другой отрицателен и само по модулю больше другого
		*this = summa( *this, other );
	} else if( negative == true && other.negative == false && !LessPoModule( *this, other ) ) {
		//если само отрицательно другой положителен и само по модулю больше другого
		*this = summa( *this, other );
	}
	Trim();
	return *this;
...
Decimal & Decimal::operator *= ( const Decimal & other ) {
	if( isZero() ) {
		negative = false; //TODO очень плохо тут!!! оно должно быть не здесь, (если ноль то знак +)
		return *this;
	}
	if( other.isZero() ) {
		*this = Decimal();
		...
		Decimal loc;
		loc = ( *this <= other ) ? MultModules( other, *this ) : MultModules( *this, other );
		loc.negative = ( negative == other.negative ) ? false : true;
		*this = loc;
	}
	return *this;
}

//так как тип имеет подобие int только большой размерности, дробная часть отбрасывается
Decimal & Decimal::operator /= ( const Decimal & other ) {
	// TODO: вставьте здесь оператор return
	//1. если знаки одинаковые
	//2. если знаки различны

	if( other.isZero() ) throw new std::runtime_error( "Divide on Zero" ); //деление на 0
	if( isZero() || LessPoModule( *this, other ) ) { //если ноль или меньше второго возвращаем ноль
		*this = Decimal();
	} else {
		Decimal loc = Divide2( *this, other );
		if( negative == true && other.negative == true ) negative = false;
		else if( negative == true && other.negative == false ) negative = true;
		else if( negative == false && other.negative == true ) negative = true;
...
void Decimal::Trim() {
	int i = count - 1;
	for( ; i >= 1; --i ) if( decm[ i ] != '0' ) break; //если все нули, то последний надо оставить
	Type * t = new Type[ i + 1 + 1 ]; //индекс отличается от кличества на 1 и еще 1 на терминатор
	if( !t ) throw new std::runtime_error( "trim" );
	int j = 0;
	for( ; j < i + 1; ++j ) t[ j ] = decm[ j ];
	t[ j ] = '\0';
	delete[ ] decm;
	decm = t;
	count = j;
...
Decimal Decimal::MultModules( const Decimal & first, const Decimal & second )const {
	if( first.isZero() || second.isZero() )
		return Decimal();
	Decimal lockRes;
	for( int i = 0; i < second.count; ++i ) {
		Decimal mul = first.MultToNumber( CharToInt( second.decm[ i ] ) );
		Decimal mulPoryadok = mul.MultPoryadok( i );
		lockRes += mulPoryadok;
	}
	return lockRes;
}
...
//
Decimal Decimal::PickOutNum( const Decimal & delimoe, const Decimal & delitel )const {
	if( delimoe < delitel ) return Decimal();
	if( delimoe == delitel ) return Decimal( delitel );
	int i = 0;
	std::string t;
	for( ; i < delitel.count; ++i ) {
...
int Decimal::NumberToMnogitel( const Decimal & pickNum, const Decimal & delitel )const {
	int res = 1;
	if( pickNum.isZero() ) return 0;
	Decimal mul = res * delitel;
	while( LessPoModule( mul, pickNum ) || EquivalentPoModule( mul, pickNum ) ) {
		++res;
		mul = res * delitel;
	}
	return res - 1;
...
Decimal Decimal::Divide( const Decimal & first, const Decimal & second )const {
	...
	//3. Вычесть произведение найденной цифры на делитель скорректированный к нужному порядку
	...
	while( delimoe > second ) {
		Decimal promeg = first.PickOutNum( delimoe, second ); //находим подчисло
		int mnogitel = first.NumberToMnogitel( promeg, second ); //находим цифру результата 
		res.push_back( IntToUChar( mnogitel ) );
		int kPoryadkaChisla = delimoe.count - promeg.count; //порядок вычитаемого числа, количество символов в самом числе за минусом тех что выбраны в подчисло
		Decimal vychitaemoe = Decimal( std::to_string( mnogitel ) ).MultPoryadok( kPoryadkaChisla ) * second; //вычитаемое из основного числа по найденной цифре результата с кореекцией на его порядок
		delimoe -= vychitaemoe;
	}
	return Decimal( res );
}

Decimal Decimal::Divide1( const Decimal & first, const Decimal & second ) const {
	std::string t;
	Decimal delimoe = first;
	while( delimoe > second ) {
...
Decimal Decimal::Divide2( const Decimal & delimoe, const Decimal & delitel ) {
	...
	std::string result;
	std::string delimoeStr = delimoe.ModulToString(); //представляем делимое в нормльном виде как строку цифр
	std::string delitelStr = delitel.ModulToString(); //представляем делитель в нормльном виде как строку цифр
	int curIndex = delitelStr.size(); //текущий индекс окончания строки делимого
	std::string pickNumber = delimoeStr.substr( 0, curIndex ); //выделяем подчисло из делимого

	if( LessPoModule( Decimal( pickNumber ), delitel ) ) { //если выделенное число меньше делителя то выделяем новое подчисло на 1 цифру больше
		pickNumber.clear();
		pickNumber = delimoeStr.substr( 0, ++curIndex );
	}
	while( LessPoModule( delitel, Decimal( pickNumber ) ) || EquivalentPoModule( delitel, Decimal( pickNumber ) ) ) { //пока подчисла делимого больше делителя
		//ТОДО должно быть сравнение на больше или равно по модулю
		int numberDelimiter = NumberToMnogitel( Decimal( pickNumber ), delitel );
		...
		while( curIndex < delimoe.ModulToString().size() && Decimal( newDelimoe ) < ModulDecimal( delitel ) ) {
			newDelimoe.push_back( delimoe[ curIndex++ ] );
			if( LessPoModule( Decimal( newDelimoe ), delitel ) ) {
				result.push_back( '0' );
			}
		}
...
Decimal Decimal::NewDelimoe( const Decimal & delimoe, const Decimal & delitel, const Decimal & subNumber, int numberAnswer )const {
	//найти индекс где начинается вторая чать подчисла
	int index1 = delimoe.count - 1;
	int index2 = subNumber.count - 1;
	while( index1 >= 0 && index2 >= 0 && ( delimoe.decm[ index1 ] == subNumber.decm[ index2 ] ) ) {
		--index1;
		--index2;
	}
	...
	//количество оставшихся элементов в исходном числе + количество полученное в подразнице
	Decimal res( ( index1 + 1 ) + x.count + 1 );
