...
void Money24B::Init( double summ ) {
  Init( summ, ( summ - static_cast< int64_t >( summ ) ) * 100 );
}
...
uint8_t Money24B::GetCop( ) const { return cop; }

double Money24B::GetSumm( ) const {
  return ( static_cast< double >( rub ) +
	   ( static_cast< double >( cop ) / 100 ) );
}
