...
static const std::string PATH = "../../stl_part5/Files/";
...
void tasks( ) {
  ...
  //****************************************************
  Modificators *modificator = new Modify14( cnt );
  //****************************************************
  //  std::for_each( cnt.begin( ), cnt.end( ),
  //                 [ & ]( type &elem ) { modificator->operator( )( elem ); }
  //                 );
  type_container cntDest;
  cntDest.resize( N );
  std::transform( cnt.cbegin( ), cnt.cend( ), cntDest.begin( ),
                  [ & ]( const type &elem ) -> type {
                    return modificator->operator( )( elem );
                  } );
  std::cout << '\n' << "**********************************" << std::endl;
  std::copy( cntDest.cbegin( ), cntDest.cend( ),
             std::ostream_iterator< type >( std::cout, " " ) );
  save_file( PATH + "output.txt", cntDest );
}
