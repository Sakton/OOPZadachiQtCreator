...
static const std::string PATH = "../../stl_part2/Files/";
...
void outFile( const std::string &name_file );
...
void outFile( const std::string &name_file, const_reference_container ctr ) {
  std::ofstream ofile( name_file );
  std::ostream_iterator< type > outIterator( ofile, "\n" );
  for ( auto &el : ctr ) *outIterator++ = el;
}
...
void tasks( reference_container ( *modificator )( reference_container c ) ) {
  ...
  std::cout << '\n'
            << "***************************************************************"
               "************"
            << std::endl;
  outFile( PATH + "output.txt", modificator( v ) );
  std::copy( v.begin( ), v.end( ),
             std::ostream_iterator< int >( std::cout, " " ) );
}
