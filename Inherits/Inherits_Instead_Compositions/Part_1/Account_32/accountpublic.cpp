...
AccountPublic::AccountPublic( const std::string &name, const std::string &acc,
                              double proc, double summ )
    : BaseAccount( name, acc, proc, summ ) {}

void AccountPublic::AddSumm( double sm ) {
  if ( sm < 0 ) {
...
void AccountPublic::SubSumm( double sm ) {
  if ( sm < 0 ) {
    std::cout << "ERROR ADDING SUM < 0";
    exit( 1 );
  }
...
void AccountPublic::AddProcent( ) {
  setSumma( getSumma( ) * getProcent( ) / 100 );
}

double AccountPublic::ToDollar( double cours ) { return getSumma( ) / cours; }

double AccountPublic::ToEuro( double cours ) { return getSumma( ) / cours; }

std::string AccountPublic::ToChislitelnoe( ) {
...
std::string AccountPublic::Sotni( int chislo ) {
  switch ( chislo ) {
    case 100:
      return "сто";
    case 200:
...
std::string AccountPublic::Desyatki( int chislo ) {
  switch ( chislo ) {
    case 10:
      return "десЯть";
    case 11:
...
std::string AccountPublic::Edinitsy( int chislo ) {
  switch ( chislo ) {
    case 1:
      return "один";
    case 2:
...
std::string AccountPublic::ParseSotni( int chislo ) {
  chislo %= 1000;
...
std::string AccountPublic::ParseThousand( int chislo ) {
